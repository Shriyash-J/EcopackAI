...
with col_head2:
    ...
    st.markdown("##### 🚀 AI-Powered Sustainable Packaging Recommendations & Analytics")
...
page = st.sidebar.radio("Go to Module:", ["🔍 Recommendation Engine", "🤖 AI Impact Predictor", "📊 BI Analytics & Reports"])

# ==============================================================================
# MODULE 1: RECOMMENDATION ENGINE
# ==============================================================================
if page == "🔍 Recommendation Engine":
    st.header("🔍 Find the Perfect Material")
    ...
    with col1:
        ...
        with st.form("recommendation_form"):
            ...
            w_eco = st.slider("🌱 Sustainability", 0.0, 1.0, 0.6)
            w_cost = st.slider("💰 Cost Efficiency", 0.0, 1.0, 0.2)
            w_strength = st.slider("💪 Durability", 0.0, 1.0, 0.2)
            
            submit_btn = st.form_submit_button("🔎 Analyze & Recommend")
    ...
    if submit_btn:
        ...
        try:
            with st.spinner(f'🤖 AI is analyzing database for {industry} materials...'):
    ...
    with col2:
        if st.session_state['ran_search'] and st.session_state['recommendations']:
            ...
            st.success(f"✅ Found {len(results)} optimal matches!")
            ...
            <div class="metric-card">
                <h3>🏆 Top Recommendation: {best_mat['material_name']}</h3>
                <p>Score: <strong>{best_mat['final_score']:.2f}</strong> | Price: <strong>₹{best_mat['price_inr_per_unit']}</strong> | CO₂ Score: <strong>{best_mat['co2_emission_score']}</strong></p>
            </div>
            """, unsafe_allow_html=True)
            st.write("") 
            ...
            # CHARTS
            tab_v1, tab_v2 = st.tabs(["📊 Market Analysis", "🕸️ Spider Chart"])
            ...
            with tab_v1:
                fig = px.scatter(
                    ...
                    labels={"price_inr_per_unit": "Price (₹)", "co2_emission_score": "CO₂ Impact"}
                )
                st.plotly_chart(fig, use_container_width=True)
            
            with tab_v2:
                st.markdown("### 🕸️ Material Capabilities")
...
# ==============================================================================
    ...
    st.header("🤖 Material Impact Predictor")
    ...
    if st.button("🚀 Run Prediction Models"):
        ...
        try:
            with st.spinner('Calculating Cost & CO₂...'):
            ...
            if response.status_code == 200:
                ...
                with m1:
                    st.markdown(f"""
                    <div class="metric-card">
                        <h4 style="color:#555">💰 Estimated Cost</h4>
                        <h1 style="color:#0d6efd">₹{preds['estimated_cost_inr']}</h1>
                ...
                with m2:
                    st.markdown(f"""
                    <div class="metric-card">
                        <h4 style="color:#555">🌍 Estimated CO₂ Score</h4>
...
# ==============================================================================
elif page == "📊 BI Analytics & Reports":
    ...
    if not st.session_state['all_materials']:
        st.warning("⚠️ Please run a search in the 'Recommendation' tab first to load materials.")
    else:
        # DATA PREPARATION
        ...
        c1, c2, c3 = st.columns(3)
        with c1:
            current_name = st.selectbox("🔻 Current Material (Baseline)", names, index=len(names)-1)
        with c2:
            new_name = st.selectbox("✅ Proposed Material (Switch)", names, index=0)
        with c3:
            annual_qty = st.number_input("📅 Annual Usage (Units)", min_value=100, value=10000, step=100)
        ...
        with kpi1:
            st.markdown(f"""<div class="metric-card"><h3>💰 Annual Cost Savings</h3>
            <h2 style="color: {'green' if cost_diff > 0 else 'red'}">₹{cost_diff:,.2f}</h2></div>""", unsafe_allow_html=True)
        with kpi2:
            st.markdown(f"""<div class="metric-card"><h3>🌍 Annual CO₂ Reduction</h3>
        ...
        with tab_c1:
            ...
            fig_trend = px.line(x=months, y=monthly_savings, markers=True, 
                                ...
                                labels={'x': 'Month', 'y': 'Total Savings (₹)'})
        ...
        with tab_c2:
            ...
            comp_data = pd.DataFrame({
                'Metric': ['Price per Unit', 'CO₂ Score'],
        ...
        # 3. EXPORT REPORTS
        st.write("---")
        st.subheader("📑 Export Sustainability Reports")
        ...
        with c_exp1:
            ...
            if st.button("📄 Generate PDF Report"):
                ...
                st.download_button(label="📥 Download PDF", data=pdf_bytes, file_name="sustainability_report.pdf", mime="application/pdf")
        ...
        with c_exp2:
            ...
            st.download_button(label="📊 Download Excel Data", data=output.getvalue(), file_name="material_analysis.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
